...
class Mutuales extends Component
{
    use WithPagination;
    ...
    public $perPage = 15;
    public $fecha, $hora;
    protected $listeners = ['eliminarMutual'];
    ...
    {
        if ($this->cobrador_id) {
            $query = Cob_mutuales::where('cobrador', $this->cobrador_id)->where('arqueo', $this->mesarqueo);
            $this->sumatotal = Cob_mutuales::where('cobrador', $this->cobrador_id)
                ->where('arqueo', $this->mesarqueo)->sum('pesos');
        } else {
            $query = Cob_mutuales::where('arqueo', $this->mesarqueo);
            $this->sumatotal = Cob_mutuales::where('arqueo', $this->mesarqueo)->sum('pesos');
        }
    ...
    public function ingresarEntrega()
    {
        ...
        $this->resetErrorBag();
        $this->reset(['importe', 'obs', 'mutual_id']);
        $this->fecha = date('Y-m-d');
        $this->hora = date('H:i');
    ...
    {
        $entrega = Cob_mutuales::find($id);
        if ($entrega) {
            $this->mutual_id = $entrega->id;
            $this->importe = $entrega->pesos;
            $this->obs = $entrega->obs;
            $this->vida = $entrega->vigilia;
    ...
    public function actualizarEntrega()
    {
        ...
        try {
            $entrega = Cob_mutuales::find($this->mutual_id);

            if ($entrega) {
                $entrega->pesos = $this->importe;
                $entrega->obs = $this->obs;
                $entrega->vigilia = $this->vida ? 1 : null;
                $entrega->save();
    ...
    public function cerrarModal()
    {
        ...
        $this->showCreateModal = false;
        $this->reset(['mutual_id', 'importe', 'obs', 'fecha', 'hora']);
        $this->resetErrorBag();
    }
    ...
    public function borrarEntrega($id)
    {
        ...
        $this->emit('confirmar-borrar-mutual', $id);
    }
